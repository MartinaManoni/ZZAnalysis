...
# Selections
'''
 if (abs(Zi_flav) == 121 and abs(Zj_flav) == 121):
        fs = '4e'  
    elif abs(Zi_flav) == 169 and abs(Zj_flav) == 169:
        fs = '4mu'     
    elif (abs(Zi_flav) == 121 and abs(Zj_flav) == 169):
        fs = '2e2mu'
    elif (abs(Zi_flav) == 169 and abs(Zj_flav) == 121):
        fs = '2mu2e'
    else:
        fs = 'other'
    return fs
...
ax[0].step(binsc, mc_tot * (sum(n_dt) / sum(mc_tot)), where='mid')
ax[0].errorbar(binsc_dt, n_dt, poisson_interval(n_dt), marker='.', color='k', linestyle='None')
